...
@register.simple_tag
def get_questions_in_activity(activity_pk):
    if ActivityQuestion.objects.filter(activity__pk=activity_pk,is_deleted=False).exists():
        activity = ActivityQuestion.objects.filter(activity__pk=activity_pk,is_deleted=False)
        return activity
    
@register.simple_tag
def get_entrolled_activities(activity_pk):
    if StudentActivity.objects.filter(activity__pk=activity_pk,is_deleted=False).exists():
        entrolled_activities = StudentActivity.objects.filter(is_completed=True,activity__pk=activity_pk,is_deleted=False)
        return entrolled_activities
